...
`define OP_J      4'b0110  // PC = {PC[15:13], imm12, 1'b0}
`define OP_JAL    4'b0111  // R15 = PC+2, PC = {PC[15:13], imm12, 1'b0}
...
`define OP_HALT   4'b1110  // halt — no operation, stops execution
...
`define R_AND     4'b0000  // R[R0] = R[rs] & R[rt]
`define R_OR      4'b0001  // R[R0] = R[rs] | R[rt]
`define R_ADD     4'b0010  // R[R0] = R[rs] + R[rt]
`define R_NOR     4'b0011  // R[R0] = ~(R[rs] | R[rt])
`define R_MFLO    4'b0100  // R[R0] = HiLo[15:0]  (lower half of multiply result)
`define R_MFHI    4'b0101  // R[R0] = HiLo[31:16] (upper half of multiply result)
`define R_SUB     4'b0110  // R[R0] = R[rs] - R[rt]
`define R_SLT     4'b0111  // R[R0] = (R[rs] < R[rt]) ? 1 : 0
`define R_XOR     4'b1000  // R[R0] = R[rs] ^ R[rt]
`define R_SLL     4'b1001  // R[R0] = R[rs] << R[rt][3:0]
`define R_SRL     4'b1010  // R[R0] = R[rs] >> R[rt][3:0]
`define R_MUL     4'b1011  // HiLo  = R[rs] * R[rt] (use MFLO/MFHI to read result)
`define R_PASSB   4'b1100  // R[R0] = R[rt]  (pass B through ALU)
`define R_PASSA   4'b1101  // R[R0] = R[rs]  (pass A through ALU)
`define R_NOT     4'b1110  // R[R0] = ~R[rs]
`define R_NEG     4'b1111  // R[R0] = -R[rs] (two's complement)
